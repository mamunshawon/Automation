...
# Define aliases for table names
table_aliases = {
    'e_money_refund': 'EMR',
    'e_money_freeze': 'EMF',
    # Add more tables with their aliases as needed
}
    ...
    if sheet_name2 in sheet_names2:
        ...
        if df2.notnull().any().any():
            ...
            for table_name, columns in table_updates.items():
                ...
                print("SET")
                updates = [f"    {alias}.{col} = NULL" if col != 'TRANSACTION_DATE_TIME' else f"    {alias}.{col} = '{transaction_date_time}'" if col == 'TRANSACTION_DATE_TIME' and alias == 'EMR' else f"    {alias}.{col} = ''" for col in columns]
                updates.append(f"    {alias}.MODIFIED_BY = {alias}.CREATED_BY")
                updates.append("    MODIFIED_BY = CREATED_BY")
                print(",\n".join(updates))
