    ...
    try:
        ...
        if status_column.text.strip() == "Active":
            ...
            if not checkbox.is_selected():
                ...
                time.sleep(2)
                Select_Auto_Settlement = driver.find_element(By.XPATH, '//*[@id="autoSettlementBankAccount"]/option[4]')
                ...
                try:
                    requester_email_element = WebDriverWait(driver, 10).until(
                        EC.presence_of_element_located(
                            (By.XPATH, "//*[contains(text(), 'mamunur.shawon@nagad.com.bd')]"))
                    )
                ...
                df = pd.read_excel(file_path)
                df.at[index, 'Status_Message'] = "Auto Settlement is already ticked"
                df.to_excel(file_path, index=False, engine='openpyxl')
                time.sleep(3)
        else:
